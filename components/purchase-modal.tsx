...
export function PurchaseModal({
  ...
  const handleJoinEvent = async () => {
    ...
    try {
      ...
      // Pass the actual event price to joinEvent
      const priceToSend =
        eventPrice === "Free"
          ? "0.00001"
          : eventPrice.replace(" MON", "").replace(" XLM", "").trim();
  ...
  const priceInMON =
    eventPrice === "Free"
      ? "0"
      : eventPrice.replace(" MON", "").replace(" XLM", "");
  const networkFee = "0.00001";
  const totalAmount =
    eventPrice === "Free"
      ? networkFee
      : (parseFloat(priceInMON) + parseFloat(networkFee)).toFixed(5);
              ...
              <>
                ...
                {!isWalletConnected && (
                  <div className="bg-orange-500/10 border border-orange-500/20 rounded-2xl p-4 mb-6">
                    ...
                    <p className="text-kaizen-gray text-xs">
                      Connect your MetaMask wallet to join this event and
                      receive rewards on Monad Testnet
                    </p>
                  </div>
                )}
            ...
            {step === "success" && (
              <div className="text-center py-8">
                ...
                <p className="text-kaizen-gray text-sm mb-4">
                  You've successfully joined {eventTitle} on Monad Testnet
                  ...
                  <div className="bg-kaizen-black/50 rounded-2xl p-3 mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            `https://testnet.monadexplorer.com/tx/${transaction.hash}`,
                            "_blank"
                          )
                        }
                      >
